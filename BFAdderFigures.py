...
def createFigure(covDict,genomeLen,numReadsPerFile,numKmersPerRead,genome_dict,MAC,MSC,genomeName):
    ...
    def addCurrent(key):
        ...
        #Plot Gx on top plot:
        ax1.plot(COV,covDict[(MAC,float('inf'))][1],"r-",label=r'$G_{'+str(MAC)+",\infty}$")

        #Plot Gx on bottom plot:
        Gx_frac_log = [ratio_1_log_scale if x==1 else -log(1-x,10) for x in covDict[(MAC,float('inf'))][2]]
        ax2.plot(COV,Gx_frac_log,"r-",label=r'$G_{'+str(MAC)+",\infty}$")

        if MSC!=float('inf'):
            #Plot Gxs on top plot:
            ax1.plot(COV,covDict[key][1],"g-",label='$G_{'+str(MAC)+","+str(MSC)+"}$")
            ...
            ax2.plot(COV,Gxs_frac_log,"g--",label='$G_{'+str(MAC)+","+str(MSC)+"}$")
    ...
    ax2.axhline(ratio_1_log_scale_2,label="99\%")
    ...
    #plt.rc('text', usetex=True)
    #plt.rc('font', family='serif')
    fig.suptitle(r"$G_{"+str(MAC)+","+str(MSC)+"}$ for genome="+str(genomeName),fontsize=16,y=1)

...
def createBarChart(maxAddCov,outDir,genomeName):
    #Read the data from the files:
    G_tot, G_per = helpers.readTotalsAndPercFromFile(outDir+"/Ginf_tot_perc.csv")
    Gx_tot, Gx_per = helpers.readTotalsAndPercFromFile(outDir+"/G"+str(maxAddCov)+"_tot_perc.csv")
    Gd_tot, Gd_per = helpers.readTotalsAndPercFromFile(outDir+"/G"+str(maxAddCov)+"d_tot_perc.csv")
    ...
    labels[0] = 'G'
    labels[1] = 'G'+str(maxAddCov)
    ax1.set_xticklabels(labels)
    
    ax1.set_ylim(0,sum(G_tot)*1.01)
    ax1.set_ylabel('Number of contigs')
    ax1.set_title('The contigs in G and G'+str(maxAddCov))
    ...
    labels[0] = 'G'+str(maxAddCov)+"d"
    ax2.set_xticklabels(labels)
    
    ax2.set_ylim(0,sum(Gd_tot)*1.01)
    ax2.set_title('The contigs in G'+str(maxAddCov)+"d")
    handles, labels = ax2.get_legend_handles_labels()
    ax2.legend(handles[::-1], labels[::-1], loc='upper right', prop={'size': 13})
  
    #Save and show the figure
    fig.suptitle("A visual representation of G, G"+str(maxAddCov)+", and G"+str(maxAddCov)+"d for genome="+genomeName)
    fig.savefig(outDir+"/"+genomeName+"_wim"+str(maxAddCov)+".png", bbox_inches='tight')
    #plt.show()

if __name__ == "__main__":
    outDir = "Output/t"
    genomeName="t"
    for maxAddCov in [5, 10, 15, 20, 30]:
        createBarChart(maxAddCov,outDir,genomeName)
